...
namespace _DSHOWLIB_NAMESPACE
{
	...
	public:
		...
		virtual tFilterError	getFilterData_( wchar_t** ppDataOut ) const
		{
			...
			*ppDataOut = (wchar_t*) ::CoTaskMemAlloc( cbSettings  );
			if( !*ppDataOut ) return eINVALID_FILTER_STATE;
	...
	protected:
		...
		*/
		...
		{
			smart_ptr<param_info<bool> > ppi = new param_info<bool>;
			...
			m_paramInfoLookup[name] = ppi;
			m_paramInfoList.push_back( ppi );
		}
		...
		*/
		...
		{
			smart_ptr<param_info<long> > ppi = new param_info<long>;
		...
		*/
		...
		{
			smart_ptr<param_info<_bstr_t> > ppi = new param_info<_bstr_t>;
		...
		*/
		...
		{
			smart_ptr<param_info<float> > ppi = new param_info<float>;
		...
		*/
		...
		{
			smart_ptr<data_info> pdi = new data_info;
	...
	private:
		...
		tParamInfoMap	m_paramInfoLookup;

		typedef std::vector<smart_ptr<param_info_base> > tParamInfoList;
		tParamInfoList	m_paramInfoList;
		...
		{
			tParamInfoMap::iterator it = m_paramInfoLookup.find( name );
			if( it != m_paramInfoLookup.end() )
		...
		virtual	bool	enumParameters_( unsigned int n, IParameterCtrl::ParamDesc* pPD ) const
		{
			...
			if( pPD == 0 )
				return false;
			if( n >= m_paramInfoList.size() )
			{
				size_t m = n - m_paramInfoList.size();
