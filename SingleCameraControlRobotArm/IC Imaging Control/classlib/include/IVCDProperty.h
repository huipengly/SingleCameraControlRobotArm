...
//	static const GUID IID_IVCDPropertyInterfaces = { 0x99B44938, 0xBFE1, 0x4083, { 0xAD, 0xA1, 0xBE, 0x70, 0x3F, 0x4B, 0x8E, 0x04 } };
	...
	// {89905350-3C1A-4009-ADA0-458A86DFBE75}
	static const GUID IID_IVCDPropertyUIInfo = { 0x89905350, 0x3c1a, 0x4009, { 0xad, 0xa0, 0x45, 0x8a, 0x86, 0xdf, 0xbe, 0x75 } };
	// {89905350-3C1A-4009-ADA0-458A86DFBE76}
	static const GUID IID_IVCDPropertyUIInfo2 = { 0x89905350, 0x3c1a, 0x4009, { 0xad, 0xa0, 0x45, 0x8a, 0x86, 0xdf, 0xbe, 0x76 } };
	...
	struct DEF_INTERFACE_GUID("89905350-3C1A-4009-ADA0-458A86DFBE75") IVCDPropertyUIInfo;
	struct DEF_INTERFACE_GUID("89905350-3C1A-4009-ADA0-458A86DFBE76") IVCDPropertyUIInfo2;
	...
	public:
		...
		template<class TItf>
		inline smart_com<TItf>& QueryInterface()
		{
			smart_com<TItf> rval;
			HRESULT hr = QueryInterface( __uuidof( TItf ), reinterpret_cast<void**>( &rval.getImpl() ) );
			if( FAILED( hr ) )
	...
	public:
		...
		template<class TItf>
		smart_com<TItf>						getInterfacePtr()
		{
			smart_com<TItf> pRval = 0;
	...
	public:		
		...
		{
			pRvalItf = 0;
			smart_com<IVCDPropertyInterface> pItf = findInterface( itemID, elementID, smart_com<TItf>::getIID() );
			if( pItf != 0 )
			{
		...
		template<class TItf>
		smart_com<TItf>	findInterfacePtr( const GUID& itemID, const GUID& elementID ) const
		{
			smart_com<TItf> pRvalItf;
			smart_com<IVCDPropertyInterface> pItf = findInterface( itemID, elementID, smart_com<TItf>::getIID() );
			if( pItf != 0 )
			{
				pItf->QueryInterface( pRvalItf );
	...
	struct IVCDPropertyUIInfo
		:	public IUnknown
	{
	public:
		STDMETHOD(get_Visibility)( long* pVisibility ) const = 0;
	public:
		enum Visibility
		{
			eAlways = 0,
	...
	public:
		Visibility getVisibilityLevel() const
		{
			Visibility vis;
			if( SUCCEEDED(get_Visibility( (long*)&vis )) )
			{
				return vis;
			}
		...
		bool isVisible( Visibility uiVisibilityLevel ) const
		{
			return getVisibilityLevel() < uiVisibilityLevel;
		}
	};

	struct IVCDPropertyUIInfo2
		:	public IVCDPropertyUIInfo
	{
	public:
		STDMETHOD(IsBuddyOf)( const GUID* pBuddyGUID, bool* pIsBuddy ) const = 0;
	public:
		bool isBuddyOf( const GUID& buddyGUID ) const
		{
			bool b;
			if( SUCCEEDED(IsBuddyOf(&buddyGUID, &b)) )
			{
				return b;
			}
