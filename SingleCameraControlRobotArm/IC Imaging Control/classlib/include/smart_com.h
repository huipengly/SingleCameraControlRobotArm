...
public:
	template<class TType>
	smart_com<TType>	QueryInterface()
	{
		return smart_com<TType>::QueryInterface(*this);
	}

	template<class TType>
	static smart_com	QueryInterface( smart_com<TType>& pObj )
	{
		smart_com rval;
		if( pObj != 0 )
			pObj->QueryInterface( rval.getIID(), (void**)&rval.get() );
		return rval;
	}
