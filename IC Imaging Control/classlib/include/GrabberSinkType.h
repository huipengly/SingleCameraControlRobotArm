...
namespace _DSHOWLIB_NAMESPACE
{
	...
	public:
		/** dtor */
		_UDSHL_EXP_API virtual ~GrabberSinkType();

		/** must return the sink id*/
		_UDSHL_EXP_API virtual tSinkType	getSinkType() const = 0;
		 ...
		 */
		_UDSHL_EXP_API virtual tSinkModes	setSinkMode( tSinkModes mode );
		/** returns the current sink mode */
		_UDSHL_EXP_API virtual tSinkModes	getSinkMode() const;
		/** Returns if the sink is currently attached to a Grabber */
		_UDSHL_EXP_API bool		isAttached() const;
