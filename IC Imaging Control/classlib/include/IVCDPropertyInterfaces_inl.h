...
namespace _DSHOWLIB_NAMESPACE
{
	...
	// methods of IVCDSwitchProperty2
	inline bool		IVCDSwitchProperty2::hasDefault()
	{
		VARIANT_BOOL def;
		return SUCCEEDED(get_Default( &def ));
	}
	inline bool		IVCDSwitchProperty2::getDefault()
	{
		VARIANT_BOOL def;
		HRESULT hr = get_Default(&def);
		if( FAILED(hr) )
			throw hr;
	...
	inline IVCDMapStringsProperty::tStringVecW	IVCDMapStringsProperty::getStringsW() const
	{
		tStringVecW vec;
		getStringsW( vec );
		return vec;
	...
	inline IVCDHexPropertyInterface::uint64_t	IVCDHexPropertyInterface::getValue() const
	{
		uint64_t rval;
		HRESULT hr = get_Value( &rval );
		if( FAILED( hr ) )	throw hr;
	...
	inline void							IVCDHexPropertyInterface::setValue( IVCDHexPropertyInterface::uint64_t val )
	{
		HRESULT hr = put_Value( val );
		if( FAILED( hr ) )	throw hr;
	}

	inline IVCDHexPropertyInterface::uint64_t	IVCDHexPropertyInterface::getMask() const
	{
		uint64_t rval;
		HRESULT hr = get_Mask( &rval );
		if( FAILED( hr ) )	throw hr;
		return rval;
	}
};
