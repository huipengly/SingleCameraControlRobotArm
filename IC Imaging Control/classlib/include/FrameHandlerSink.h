...
namespace _DSHOWLIB_NAMESPACE
{
	...
	public:
		...
		struct tCreateData {
			...
			FrameTypeInfoArray				getFrameTypeInfoArray() const {
				FrameTypeInfoArray tmp;
				tmp.assign( m_FrameTypes.begin(), m_FrameTypes.end() );
				return tmp;
			}
		...
		_UDSHL_EXP_API static tFHSPtr	create( const tCreateData& data );
	public:
		_UDSHL_EXP_API ~FrameHandlerSink();
		 ...
		 */
		_UDSHL_EXP_API void		setSnapMode( bool bSnapModeOn );
		_UDSHL_EXP_API bool		getSnapMode() const;
		 ...
		 * @return
		 */
		_UDSHL_EXP_API Error		snapImages( unsigned int count, DWORD timeout = INFINITE );
		/** Enqueues a snap job, which lets <count> frames to be copied into the MemBufferCollection.
		 ...
		 * @return
		 */
		_UDSHL_EXP_API Error		snapImagesAsync( unsigned int count );
		 ...
		 */
		_UDSHL_EXP_API smart_ptr<MemBufferCollection>	getMemBufferCollection() const;
		 ...
		 */
		_UDSHL_EXP_API smart_ptr<MemBuffer>			getLastAcqMemBuffer() const;
		 ...
		 */
		_UDSHL_EXP_API bool							getOutputFrameType( FrameTypeInfo& info ) const;
		 ...
		 */
        _UDSHL_EXP_API bool							setMemBufferCollection( const smart_ptr<MemBufferCollection>& pCol );
		...
		_UDSHL_EXP_API unsigned int					getFrameCount() const;
